...
namespace Ifm.Components.Messenger.Blocks.Utilities
{
    internal class HelperClass
    {
        ...
        #region Constructors
        public HelperClass(dynamic voideHandler, string serviceName)
        {
            mHandler = voideHandler;
        ...
        public string GetParamValue(string name)
        {
            ...
            if (mHandler != null)
            {
                ...
                LogString(string.Format("GetParamValue - Name = {0} - Value = {1}", name, rc));
            }

            return rc;
        }

        public void SetTransferPropertyValue(string name, string value)
        {
            if (mHandler != null)
            {
                LogString(string.Format("Setting TransferPropertyValue: Name='{0}', Value='{1}'", name, value));
                mHandler.TransferPropertyValue[name] = value;
            }
        }

        public string GetTransferPropertyValue(string name)
        {
            string tpValue = "";
            ...
            if (mHandler != null)
            {
                ...
                LogString(string.Format("Get TransferPropertyValue: Name='{0}', Value='{1}'", name, tpValue));
            }
        ...
        public void SetCallDataValue(string name, string value)
        {
            if (mHandler != null)
            {
                LogString(string.Format("Setting CallData Value: Name='{0}', Value='{1}'", name, value));
                mHandler.monitorManager.currentCall.callData.setValue(name, value);
            }
        }

        public string GetCallDataValue(string name)
        {
            string cdValue = "";
            ...
            if (mHandler != null)
            {
                ...
                LogString(string.Format("Get CallData Value: Name='{0}', Value='{1}'", name, cdValue));
            }
                    ...
                    else
                    {
                        switch (gr.GreetingType)
                        {
                            case CGreeting.GreetingTypes.Wav:
                                LogString(string.Format("PlayGreeting wav = {0}", gr.Message));
